...
export function collectAllDataFromPage() {
    ...
    const prescriptionForms = form.querySelectorAll("#prescriptions-list form#prescription-form");
    const prescriptions = Array.from(prescriptionForms).map((prescriptionForm) => {
    ...
    return {
        ...
        expirationDate: prescriptionData.get("med-prescription-expiration") || "",
        amount: {
            value: prescriptionData.get("med-prescription-amount") || "",
            unit: prescriptionData.get("med-prescription-amount-unit") || "",
        },
        repetitions: prescriptionData.get("med-prescription-repetitions") || "",
        total: prescriptionData.get("med-prescription-total") || "",
        nextOrderDate: prescriptionData.get("med-prescription-next-order") || "",
        status: fieldset && fieldset.classList.contains('inactive') ? 'ended' : 'active',
    }
});
